...
export default function StudentHomePage() {
  ...
  return <AppShell active="/dashboard/home" rightRail={<RightRail bookings={bookingData?.items ?? []} completed={dashboard?.completed_bookings ?? 0} />}>
    <div className={styles.stack}>
      ...
      <Section title="Popular Course" href="/mentors"><div className={styles.courseGrid}>{courses.map(([type,title,image])=><article className={styles.course} key={title}><div><Image src={`/assets/app/${image}`} alt="" fill sizes="(max-width:767px) 30vw, 220px" /></div><small>{type}</small><h3>{title}</h3><p>Published course</p></article>)}{courses.length === 0 && <p className="data-state">No published courses yet.</p>}</div></Section>
      <Section title="Top Mentors" href="/mentors"><div className={styles.mentorGrid}>{mentors.map(([role,image,name,id])=><article key={id}><Image src={`/assets/app/${image}`} alt={name} width={62} height={62} /><div><h3>{name}</h3><p>♛ Verified <span>{role}</span></p></div><Link href={`/mentors/${id}`} aria-label={`View ${name}, ${role}`}>♟</Link></article>)}{mentors.length === 0 && <p className="data-state">No approved mentors yet.</p>}</div></Section>
